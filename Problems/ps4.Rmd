...
- Include complete, well-commented R code.
...
**BLP Approximation to Nonlinear CEFs**

**2a.** The lecture slides show a nonlinear CEF for GDP and turnout, with different linear approximations (BLPs) for different ranges of GDP. In your own words, explain:
...
**2b.** Using the turnout data from the lecture, create a visualization that demonstrates this phenomenon:
...
# Create three subsets: low, medium, and high GDP
turnout_low <- turnout %>% filter(GDP < 1e7)
turnout_medium <- turnout %>% filter(GDP >= 1e7 & GDP < 2e7)
turnout_high <- turnout %>% filter(GDP >= 2e7)

# Fit linear models for each subset
lm_low <- lm(Turnout ~ GDP, data = turnout_low)
lm_medium <- lm(Turnout ~ GDP, data = turnout_medium)
lm_high <- lm(Turnout ~ GDP, data = turnout_high)

# Create a plot showing all data points and the three regression lines
library(ggplot2)
ggplot(turnout, aes(x = GDP, y = Turnout)) +
  geom_point(alpha = 0.5) +
  geom_smooth(data = turnout_low, method = "lm", se = FALSE, color = "red", size = 1) +
  geom_smooth(data = turnout_medium, method = "lm", se = FALSE, color = "blue", size = 1) +
  geom_smooth(data = turnout_high, method = "lm", se = FALSE, color = "green", size = 1) +
  geom_smooth(method = "lm", se = FALSE, color = "black", size = 1.5, linetype = "dashed") +
  labs(title = "BLP Approximations to Nonlinear CEF at Different GDP Ranges",
       subtitle = "Red: Low GDP, Blue: Medium GDP, Green: High GDP, Black: Overall BLP") +
  theme_minimal()
```

**2c.** Interpret the coefficients from each of the four regressions. How do they differ? What does this tell us about interpreting a single linear coefficient when the relationship is nonlinear?
...
**OVB in Your Turnout Analysis**

Return to your voter turnout analysis from Problem Set 3.

**3a.** Re-examine the difference between your bivariate model (turnout ~ income) and multivariate model (turnout ~ income + religiosity + age). Calculate the omitted variable bias using the formula from Problem 1.
...
1. What is the true value of β₁?
2. What is the estimated β₁ in the bivariate model? How biased is it?
...
**Matrix Algebra and OLS Derivation**
...
2. Using the plug-in principle, show how to derive the OLS estimator for β₁ in the simple regression model: $y = β₀ + β₁x + ε$
...
lm(Turnout ~ GDP + Temperature, data = turnout_clean)
```

**Questions for 5b:**
1. Verify that your matrix calculation matches the `lm()` output.
2. What does each component of the formula $(X^TX)^{-1}X^Ty$ represent intuitively?
3. What happens if $X^TX$ is not invertible? What does this mean in practical terms?

**5c. Connecting Matrix Form to OVB:**
The OVB formula can also be expressed in matrix terms. For a model with two predictors:

$$\hat{\beta}_1 = \beta_1 + \beta_2 \frac{\text{Cov}(x_1, x_2)}{\text{Var}(x_1)}$$

Show how this emerges from the matrix formula when we omit $x_2$.