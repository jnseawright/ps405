...
- For Problems 1-3, provide clear explanations in your own words.
...
**1b.** Using the election data from the lectures:
...
**Matrix Algebra and Multicollinearity**
  
  **2a. Matrix Form:**
1. Write down the regression model in matrix form and explain each symbol.

**2b. Implementing OLS with Matrix Algebra:**
...
Y <- as.matrix(turnout_clean$Turnout)

# Calculate OLS coefficients using matrix algebra
beta_hat <- solve(t(X) %*% X) %*% t(X) %*% Y

# Compare with lm()
model_lm <- lm(Turnout ~ GDP + Temperature, data = turnout_clean)
```

**Questions:**
  1. Verify that your matrix calculation matches the `lm()` output.
2. What is the condition for $(X^TX)$ to be invertible?

  **2c. Multicollinearity Investigation:**
  Create a multicollinear scenario:
  
```{r problem2c, eval=FALSE}
