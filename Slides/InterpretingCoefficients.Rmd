...
x_vals <- seq(min(election2016$TrumpShare), max(election2016$TrumpShare), length.out = 30)
y_vals <- seq(min(election2016$D12Margin), max(election2016$D12Margin), length.out = 30)

grid_data <- expand.grid(TrumpShare = x_vals, D12Margin = y_vals)
grid_data$Stateterrorism <- predict(blp.multivariate, newdata = grid_data)

z_matrix <- matrix(grid_data$Stateterrorism, 
                   nrow = length(y_vals),  # Rows = TrumpShare
                   ncol = length(x_vals),  # Columns = D12Margin
...
![Coefficients 1](Images/Krause1.png)

---

![Coefficients 2](Images/Krause2.png)

---

![Coefficients 3](Images/Krause3.png)

---

![Coefficients 4](Images/Krause4.png)